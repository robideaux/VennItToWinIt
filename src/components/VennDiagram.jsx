...
// Hit-test a label pill (60px wide, 20/30px tall) centered at (cx,cy), plus touch margin
function hitsPill(x, y, cx, cy, label, margin = 5) {
  const halfW = 30 + margin
  const halfH = (label.split(' ').length > 1 ? 15 : 10) + margin
      ...
      <g style={{ isolation: 'isolate' }}>
        {CIRCLES.map(c => (
          <circle
            ...
            fillOpacity={0.38}
            stroke={revealMap[c.id] ? c.color : '#adb5bd'}
            strokeWidth={2}
